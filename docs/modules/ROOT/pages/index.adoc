...
If you want to use this extension, you need to add the `io.quarkiverse.signals:quarkus-signals` extension first to your build file.
...
----
<dependency>
    <groupId>io.quarkiverse.signals</groupId>
...
The method can return the value directly or wrapped in a `Uni`:
...
* A method returning `Uni` defaults to `NON_BLOCKING` -- when Vert.x is available, it is executed on the event loop.
...
include::includes/quarkus-signals.adoc[leveloffset=+1, opts=optional]
